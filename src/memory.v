...
//
// Invariant:
// - If a TLB fault arrives (`tlb_exc_in`), this stage synthesizes a dedicated
//   live exception slot that carries the faulting PC/address and suppresses all
//   younger memory-side effects. The ISA requires EPC/EFG/cr7 to describe the
//   trapping memory instruction, not the younger slot currently in execute.
module tlb_memory(
    ...
    input [3:0]flags_in,
    input [31:0]op1, input [31:0]op2, input no_alias_in_1,
    input exc_in_wb, input rfe_in_wb,
  ...
  wire tlb_fault_live = (tlb_exc_in == 8'h82) || (tlb_exc_in == 8'h83);
  ...
  // Preserve the trapping slot's PC/flags so exception entry snapshots the
  // correct architectural state instead of a younger execute-stage slot.
  reg [31:0]fault_pc_buf;
  reg [3:0]fault_flags_buf;
    ...
    if (clk_en) begin
      if (halt) begin
        ...
        if (!bubble_in && (is_load || is_store)) begin
          ...
          fault_pc_buf <= exec_pc_out;
          fault_flags_buf <= flags_in;
        end

        if (tlb_fault_live) begin
          tgt_out_1 <= 5'd0;
          tgt_out_2 <= 5'd0;
          opcode_out <= 5'd0;
          result_out_1 <= 32'd0;
          result_out_2 <= 32'd0;
          bubble_out <= (exc_in_wb || rfe_in_wb || halt) ? 1'b1 : 1'b0;
          addr_out <= fault_addr_buf;

          mem_pc_out <= fault_pc_buf;
          exc_out <= tlb_exc_in;
          tgts_cr_out <= 1'b0;
          priv_type_out <= 5'd0;
          crmov_mode_type_out <= 2'd0;
          flags_out <= fault_flags_buf;
          ...
          bubble_out <= (exc_in_wb || rfe_in_wb || halt) ? 1'b1 : bubble_in;
          ...
          no_alias_out_1 <= bubble_in ? 1'b0 : no_alias_in_1;
